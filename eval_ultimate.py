...
def cleanup_after_syntheval(dataset):
    matched_files = glob.glob('*SE_benchmark_ranking_*.csv')
    original_file = matched_files[0]
    df = pd.read_csv(original_file)
    os.remove(original_file)
    df.to_csv(f'{dataset}_benchmark.csv', index=False, sep=';', decimal=',')
    matched_files = glob.glob('*SE_benchmark_results_*.csv')
    original_file = matched_files[0]
    os.remove(original_file)

datasets = ["pima", "diabetic", "heart", "playground"]
targets = ["Outcome", "TYPE", "DEATH_EVENT", "Kfold"]

for dataset, target in datasets, targets:
