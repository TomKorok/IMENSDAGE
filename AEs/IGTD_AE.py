        ...
        class encoder(nn.Module):
            def __init__(self, n_features):
                ...
                self.lin_encoder = nn.Sequential(
                    nn.Linear(self.n_features, self.img_size * self.img_size),
                    nn.LeakyReLU(0.2, inplace=True),
                )

                self.conv_encoder = nn.Sequential(
                    nn.ConvTranspose2d(1, 4096, kernel_size=(ksp_h["k1"], ksp_w["k1"]), stride=(ksp_h["s1"], ksp_w["s1"]), padding=(ksp_h["p1"], ksp_w["p1"])),
                    nn.BatchNorm2d(4096),
                    nn.LeakyReLU(0.2, inplace=True),

                    nn.ConvTranspose2d(4096, 1, kernel_size=(ksp_h["k2"], ksp_w["k2"]), stride=(ksp_h["s2"], ksp_w["s2"]), padding=(ksp_h["p2"], ksp_w["p2"])),
                    nn.Tanh(),
                )
                self.optimizer = optim.Adam(self.parameters(), lr=0.00001)

            def forward(self, x, labels=None):
                x = self.lin_encoder(x).view(-1, 1, self.img_size, self.img_size)
        ...
        class decoder(nn.Module):
            def __init__(self, n_features):
                ...
                self.conv_decoder = nn.Sequential(
                    nn.Conv2d(1, 4096, kernel_size=(ksp_h["k2"], ksp_w["k2"]), stride=(ksp_h["s2"], ksp_w["s2"]), padding=(ksp_h["p2"], ksp_w["p2"])),
                    nn.BatchNorm2d(4096),
                    nn.LeakyReLU(0.2, inplace=True),

                    nn.Conv2d(4096, 4096, kernel_size=(ksp_h["k1"], ksp_w["k1"]), stride=(ksp_h["s1"], ksp_w["s1"]), padding=(ksp_h["p1"], ksp_w["p1"])),
                    nn.BatchNorm2d(4096),
                    nn.LeakyReLU(0.2, inplace=True),
                    ...
                    nn.Linear(4096 * self.img_size * self.img_size, self.n_features),
                )
                self.optimizer = optim.Adam(self.parameters(), lr=0.00001)
