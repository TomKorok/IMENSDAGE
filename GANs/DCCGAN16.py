...
class DCC_Generator16(nn.Module):
    def __init__(self, nz, n_classes):
        ...
        self.conv_generator = nn.Sequential(
            ...
            nn.ConvTranspose2d(self.channel_multiplier * 16, 3, 4, 2, 1, bias=False),
            nn.Tanh()
...
class DCC_Discriminator16(nn.Module):
    def __init__(self, n_classes):
        ...
        self.conv_discriminator = nn.Sequential(
            # input is `(4) x 16 x 16`
            nn.Conv2d(4, self.channel_multiplier * 8, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. `(channel_multiplier * 8) x 8 x 8`
            nn.Conv2d(self.channel_multiplier * 8, self.channel_multiplier * 16, 4, 2, 1, bias=False),
            nn.BatchNorm2d(self.channel_multiplier * 16),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. `(channel_multiplier * 16) x 4 x 4``
            nn.Conv2d(self.channel_multiplier * 16, 1, 4, 1, 0, bias=False),
            nn.Sigmoid()
        )
